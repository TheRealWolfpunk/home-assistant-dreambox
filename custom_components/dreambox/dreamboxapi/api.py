...
from .data import Service, ServiceList, DeviceInfo, SimpleResult, Volume
...
class DreamboxApi(object):
    ...
    def set_channel_up(self):
        self.set_remotekeypress(DreamboxApi.KEY_CHANNELUP)

    def set_channel_down(self):
        self.set_remotekeypress(DreamboxApi.KEY_CHANNELDOWN)
    ...
    def set_powerstate(self, state):
        ...
        if root is None:
            return
        in_standby = root.find("e2instandby") if root is not None else None
        self._instandby = (
            (in_standby.text or "").lower() == "true" if in_standby is not None else False
        )
    ...
    def set_remotekeypress(self, code):
        root = self._call(
            self._url(DreamboxApi.URL_REMOTECONTROL), data={"command": code}
        )
        if root is None:
            return False
